...
export async function GET(req: NextRequest) {
  ...
  try {
    ...
    const partida = Array.isArray(worldsRaw.partida)
      ? worldsRaw.partida.map((n) => Number(n)).filter((n) => Number.isFinite(n) && n >= 1 && n <= 5)
      : [];
...
// POST /api/worlds
//   completar-mision  {mision: 1..5}  — autenticado; valida secuencia, otorga
//     los puntos de la misión una sola vez y desbloquea el Tablero al cerrar
//     la Calibración Inicial (misión 5).
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    if (accion === 'completar-mision') {
      ...
      const tablero = mision === 5 ? true : worldsRaw.tablero === true;
      ...
      return NextResponse.json({
        ...
        desbloqueo: tablero ? (mision === 5 ? { tablero: true } : {}) : {},
      });
    }
