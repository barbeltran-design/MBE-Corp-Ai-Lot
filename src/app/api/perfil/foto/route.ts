...
export async function POST(req: NextRequest) {
  try {
    ...
    if (!res.ok || !result.secure_url) {
      console.error('[perfil/foto] cloudinary error', result.error?.message || result);
      return NextResponse.json({ error: 'El proveedor de imágenes rechazó la subida.' }, { status: 502 });
    }
